...
# Load Metadata
# This file is retrieved from GDC portal and maps individual GDC files to patient samples

metaMatrix.RNA<- read.csv("metadata/BRCA_metadata.csv", row.names=1, header=TRUE,check.names=TRUE)

# Execute Merging Pipeline
# Converts individual downloads into a unified matrix (e.g., TPM/Counts)
...
write.csv(rnaCounts, file = "results/BRCA_Expression_Matrix.csv")
